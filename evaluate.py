...
def evaluate(embeds, idx_train, idx_val, idx_test, labels, task,epoch,lr ,isTest=True, iterater=10,losstype='nowhite'):
    ...
    if task == 'Node':
        if isTest:
            print("\t[Classification] Macro-F1: {:.4f} ({:.4f}) | Micro-F1: {:.4f} ({:.4f})".format(
                macro_f1, np.std(macro_f1s), micro_f1, np.std(micro_f1s)))
